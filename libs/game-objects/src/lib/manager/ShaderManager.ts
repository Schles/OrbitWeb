...
export class ShaderManager {
  ...
  constructor(private gameManager: GameManager) {
    ...
    resources = this.prototypes.reduce((acc, cur) => {
      if (acc.findIndex((v) => v === cur.shader.v) < 0) acc.push(cur.shader.v);

      if (acc.findIndex((v) => v === cur.shader.f) < 0) acc.push(cur.shader.f);
    ...
    this.gameManager.loader.load((loader, res) => {
      for (let key in res) {
        ...
        this.replacements
          .filter((r) => r.source.indexOf(key) >= 0)
          .forEach((r) => {
  ...
  public createFilter(name: string, options): Filter {
    ...
    if (!shaderProto) return undefined;
