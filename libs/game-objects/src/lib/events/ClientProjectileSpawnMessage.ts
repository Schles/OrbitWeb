  ...
  onRecieve(context: GameManager) {
    ...
    if (source !== undefined) {
      ...
      if (this.message.projType === 'rocketProjectile') {
        ...
        projectileGO = new Mine(this.message.id, source, {x: this.message.x, y: this.message.y});
      } else if (this.message.projType === 'laserProjectile') {
        projectileGO = new Laser(this.message.id, source, {x: this.message.x, y: this.message.y});
      } else {
